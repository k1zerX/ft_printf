...
/*   Updated: 2019/09/07 21:45:59 by kbatz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	f_counter(char *nbr, t_format f)
{
	...
	if (f.width < 0)
		f.width = 0;
	str = malloc((f.plus + f.width + len + f.precision + (f.precision == 0 && f.sharp)) * sizeof(*str));
	if (!str)
		exit(1);
	ft_floatfill(str, nbr, f, len);
	write(1, str, f.plus + f.width + len + f.precision + (f.precision == 0 && f.sharp));
	free(str);
	free(nbr);
	return (f.plus + f.width + len + f.precision + (f.precision == 0 && f.sharp));
}
